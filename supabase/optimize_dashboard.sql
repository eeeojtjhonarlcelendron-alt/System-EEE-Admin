...
-- Step 7: Create RPC function for KPI metrics by hub (aggregated in SQL, not JavaScript)
-- Returns CFR and Scorecard metrics for each hub without fetching raw records
CREATE OR REPLACE FUNCTION get_kpi_metrics_by_hub(hub_filter TEXT DEFAULT NULL, from_date DATE DEFAULT NULL, to_date DATE DEFAULT NULL)
RETURNS TABLE (
  hub VARCHAR,
...
LANGUAGE SQL
AS $$
  WITH filtered_kpi AS (
    SELECT
      operator_hub,
      CAST(cfr AS DECIMAL) as cfr_val,
      CAST(score AS DECIMAL) as score_val,
      date
    FROM kpi_records
    WHERE (hub_filter IS NULL OR operator_hub = hub_filter)
  ...
  SELECT
    ...
    ROUND(AVG(cfr_val)::DECIMAL, 2) as clear_floor_rate,
    ROUND(AVG(score_val)::DECIMAL, 2) as scorecard,
    ROUND(AVG(score_val)::DECIMAL, 2) as avg_score,
    ROUND(MAX(cfr_val)::DECIMAL, 2) as max_cfr,
    ROUND(MIN(cfr_val)::DECIMAL, 2) as min_cfr,
    MAX(date) as latest_date
  FROM filtered_kpi
  GROUP BY operator_hub
  ORDER BY clear_floor_rate DESC;
$$;

-- Step 8: Create RPC function for overall KPI metrics (no raw data fetch needed)
-- Returns aggregated KPI metrics across all hubs
CREATE OR REPLACE FUNCTION get_overall_kpi_metrics(from_date DATE DEFAULT NULL, to_date DATE DEFAULT NULL)
RETURNS TABLE (
  clear_floor_rate DECIMAL,
  scorecard DECIMAL,
  avg_score DECIMAL,
...
AS $$
  SELECT
    ROUND(AVG(CAST(cfr AS DECIMAL))::DECIMAL, 2) as clear_floor_rate,
    ROUND(AVG(CAST(score AS DECIMAL))::DECIMAL, 2) as scorecard,
    ROUND(AVG(CAST(score AS DECIMAL))::DECIMAL, 2) as avg_score,
    COUNT(*)::INTEGER as total_records,
    COUNT(DISTINCT operator_hub)::INTEGER as total_hubs,
    MAX(date) as latest_date
  FROM kpi_records
...
COMMENT ON FUNCTION get_kpi_metrics_by_hub(TEXT, DATE, DATE) IS 
  'Returns aggregated KPI metrics by hub. Eliminates need to fetch raw kpi_records and aggregate in JavaScript.';

COMMENT ON FUNCTION get_overall_kpi_metrics(DATE, DATE) IS 
  'Returns overall aggregated KPI metrics across all hubs. Single SQL query replaces fetching all raw records.';
