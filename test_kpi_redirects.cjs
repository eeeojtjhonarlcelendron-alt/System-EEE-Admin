...
function followRedirect(url, maxRedirects = 10, depth = 0) {
  ...
  console.log(`Request ${depth + 1}: ${url.substring(0, 80)}...`);

  https.get(url, (res) => {
    console.log(`  Status: ${res.statusCode}`);

    if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
      console.log(`  → Redirects to: ${res.headers.location.substring(0, 80)}...`);
    ...
    if (res.statusCode === 200) {
      ...
      res.on('end', () => {
        console.log(`\n✓ Got response (${data.length} bytes)`);
        try {
          ...
          if (Array.isArray(json)) {
            ...
            if (json.length > 0) {
              console.log('Sample keys:', Object.keys(json[0]).slice(0, 5).join(', '));
            }
          }
        } catch (e) {
...
console.log('Testing KPI Endpoint with Redirect Following...\n');
